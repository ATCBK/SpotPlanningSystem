...
describe('spots dataset', () => {
  it('expands the scenic spot overview to seventeen attractions across seven cities', () => {
    expect(spots).toHaveLength(17)
    expect(spots.map((spot) => spot.name)).toEqual(
      expect.arrayContaining(['二七纪念塔', '洛阳博物馆', '铁塔公园', '卧龙岗武侯祠', '西河古村']),
    )
    expect(new Set(spots.map((spot) => spot.city))).toEqual(
      new Set(['郑州', '洛阳', '开封', '安阳', '焦作', '南阳', '信阳']),
    )
  })
